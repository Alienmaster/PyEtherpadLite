...
class EtherpadLiteClient:
    ...
    def checkValidFormat(self, dataType, param):
        """Format check for dataTypes
        see: https://etherpad.org/doc/v1.8.16/#index_data-types"""
        if not isinstance(param, str):
            raise ValueError('parameter ' + dataType + ' needs to be a String.')
        if dataType == 'authorID':  # TODO change order in Docs https://etherpad.org/doc/v1.8.16/#index_data-types
            pattern = re.compile(r"a.[A-Za-z0-9]{16}")
        elif dataType == 'groupID':
            pattern = re.compile(r"g.[A-Za-z0-9]{16}")
        elif dataType == 'padID':  # padID refers _only_ to Group Pads (confusing. i know)
            pattern = re.compile(r"g.[A-Za-z0-9]{16}\$[A-Za-z0-9_]+")
        elif dataType == 'readOnlyID':
            pattern = re.compile(r"r.[A-Za-z0-9]{16}")
        elif dataType == 'sessionID':
            pattern = re.compile(r"s.[A-Za-z0-9]{16}")

        # Checks parameter
        if not re.fullmatch(pattern, param):
            raise ValueError("parameter " + dataType + " is in a wrong format")  # TODO more specific
    ...
    def getChatHistory(self, padID, start=None, end=None):
        """returns a part of the chat history, when start and end are given or
    ...
    def appendChatMessage(self, padID, text, authorID, time=''):
        """creates a chat message, saves it to the database and
        sends it to all connected clients of this pad"""
